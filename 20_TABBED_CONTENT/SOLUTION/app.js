...
const initialEpisode = 38;
const tabsElement = document.querySelector('#tabs');
const contentElement = document.querySelector('.content');
const coverElement = document.querySelector('.cover img');

function showEpisode({ title, description, cover, link }) {
  contentElement.querySelector('h1').innerText = title;
  contentElement.querySelector('p').innerText = description;
  contentElement.querySelector('a').href = link;

  coverElement.src = `./images/${cover}`;
}
...
function handleTabClicked(e) {
  ...
  const episode = episodes.find((episode) => episode.id === +this.dataset.episodeId);
  showEpisode(episode);
}

function initTabs() {
  tabsElement.innerHTML = episodes
    .map(
      ({ id, title }) =>
        `<li class="tab ${id === initialEpisode ? 'selected' : ''}">
        <a href="#" data-episode-id="${id}">
          <div class="episode">Episode ${id}</div>
          <div class="title">${title}</div>
        </a>
      </li>`
    )
    .join('');
  showEpisode(episodes.find(({ id }) => id === initialEpisode));
  document.querySelectorAll('.tab a').forEach((tab) => tab.addEventListener('click', handleTabClicked));
}
