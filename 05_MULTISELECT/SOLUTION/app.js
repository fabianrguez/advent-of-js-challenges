...
const buttonAllPlayed = document.querySelector('.btn-all-played');
...
function handleCheckbox(e) {
  ...
  if (e.shiftKey && this.checked) {
    checkboxes.forEach((checkbox) => {
      if (this === checkbox || checkbox === lastCheckboxChecked) {
        inBetween = !inBetween;
      }
...
function toggleAllPlayed() {
  const { allMarked } = this.dataset;
  let areAllCheched = true;
  let buttonText = 'Mark as unplayed';
  
  if (allMarked === 'true') {
    areAllCheched = false;
    buttonText = 'Mark as played';
  }
  this.dataset.allMarked = areAllCheched;
  this.innerText = buttonText;

  checkboxes.forEach((checkbox) => (checkbox.checked = areAllCheched));
}

checkboxes.forEach((checkbox) => checkbox.addEventListener('click', handleCheckbox));
buttonAllPlayed.addEventListener('click', toggleAllPlayed);
